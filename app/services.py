...
from . import (db, ingest_aircraft_types, ingest_airports, ingest_faa, ingest_faa_airports,
               ingest_photos, ingest_t100)
...
SERVICES: list[ServiceSpec] = [
    ...
    ServiceSpec("planespotters_photos", "Aircraft Photos",
                "Self-hosted aircraft photos (large thumbnails) by tail. Source: Planespotters public API (jets first; 250/run rate-limited — backfills the fleet over ~24h, then idles on new tails only).",
                "ingestion", 1800, ingest_photos.ingest_photos),
